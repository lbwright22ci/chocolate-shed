...
class WorkshopTypeAdmin(admin.ModelAdmin):
    """ Renders all instances of :model:`WorkshopType` in admin
      panel"""
    list_display = ('target_audience', 'workshop_duration',
                    'workshop_price')
...
class WorkshopActivityAdmin(SummernoteModelAdmin):
    """ Renders all instances of :model:`WorkshopActivity` in the
      admin panel
    """
...
class WorkshopAdmin(admin.ModelAdmin):
    ...
    Site administrator can filter workshops by category and
    publication status
    ...
    list_display = ('event_date', 'category__target_audience',
                    'activity__session_name', 'tickets_sold')
