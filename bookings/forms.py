...
RATING = ((1, 'Terrible'), (2, 'Disappointing'), (3, 'Average'),
          (4, 'Enjoyed it!'), (5, 'Brilliant!'))


class CustomSignupForm(forms.Form):
    """
    Customises AllAuth signup form to include the additional fields
      'first name', last name' for :model:`auth.User`
    ...
    def signup(self, request, user):
        ...
        profile.newsletter_consent =\
        self.cleaned_data.get('newsletter_consent', False)
        profile.save()
...
class WorkshopActivityForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['activity'].queryset = WorkshopActivity.objects.filter(
            activity__publication_status=1).distinct("session_name")


class ReservationForm(forms.ModelForm):
    """
    Creates :form: from the :model:`bookings.Reservation`
    Fields collected by the form are 'workshop', 'tickets',
      'has_dietary_requirements', 'additional_information' and 'consent_given`
    """
    class Meta:
        model = Reservation
        fields = ['workshop', 'tickets', 'has_dietary_requirements',
                  'additional_information', 'consent_given']

    def __init__(self, *args, **kwargs):
        # ensures that workshops offered are only those which match the
        #  activity name previously selected by the customer
        ...
        self.fields['workshop'].queryset = Workshop.objects.filter(
            activity=workshop_name, publication_status=1)
        self.fields['additional_information'].widget = forms.Textarea(
            attrs={'rows': 4})
...
class FeedbackForm(forms.ModelForm):
    ...
    Fields collected by the form are 'feedback_rating', 'feedback_comment'
      and 'recommend'
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['feedback_comment'].widget = forms.Textarea(attrs={
                                                                'rows': 5})
