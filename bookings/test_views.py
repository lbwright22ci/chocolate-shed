...
from .forms import UserProfileForm, ReservationForm, FeedbackForm, WorkshopActivityForm
...
class TestBookingsViews(TestCase):
    def setUp(self):
        ...
        self.superuser = User.objects.create_user(username='adminstaff',
                                                    email='test@admin.com', first_name='admin',
                                                    last_name='staff', is_staff=True, password='passwordabc')
        self.userprofile = UserProfile(user=self.user, newsletter_consent = True,
                                       staff_status = True)
        self.userprofile.save()
        self.userprofileadmin = UserProfile(user=self.superuser, newsletter_consent = True,
                                            staff_status = True)
        ...
        self.workshopactivity = WorkshopActivity(session_name="open child workshop",
                                excerpt="a workshop", full_description="" \
                                "long description")
        self.workshopactivity.save()
        self.workshopactivityfamily = WorkshopActivity(session_name="open family workshop",
                                excerpt="a workshop", full_description="" \
                                "long description")
        self.workshopactivityfamily.save()
        self.workshopactivityclosed = WorkshopActivity(session_name="closed child workshop",
                                excerpt="a workshop", full_description="" \
                                "long description")
        self.workshopactivityclosed.save()
        self.workshopopen = Workshop(category=self.workshoptype, activity=
                                 self.workshopactivity, slug="workshop-slug-open",
                                 location="on site", low_stock=False,
                                 tickets_sold=2, 
                                 publication_status=1, event_date=timezone.now()+timedelta(days=30))
        self.workshopopen.save()
        self.workshopopenfamily = Workshop(category=self.workshoptypefamily, activity=
                                 self.workshopactivityfamily, slug="workshop-slug-open-family",
                                 location="on site", low_stock=False, 
                                 tickets_sold=0,
                                 publication_status=1, event_date=timezone.now()+timedelta(days=31))
        self.workshopopenfamily.save()
        self.workshopclosed = Workshop(category=self.workshoptype, activity=
                                 self.workshopactivityclosed, slug="workshop-slug-closed",
                                 location="on site", low_stock=False, 
                                 publication_status=3, event_date=timezone.now()-timedelta(days=20))
        self.workshopclosed.save()
        self.pendingbooking = Reservation(workshop=self.workshopopen, customer=self.user,
                                          tickets =2, consent_given=True,
                                          has_dietary_requirements=True,
                                          additional_information='reservation pending shows')
        self.pendingbooking.save()
        self.oldbooking = Reservation(workshop=self.workshopclosed, customer = self.user,
    ...
    def test_settingsaccountpage_renders_correctly(self):
        ...
        self.assertIn(b"You are subscribed to our mailing list", response.content)
    ...
    def test_newsletter_status_updated_correctly(self):
        """ Tests that logged in user can update their mailing list preferences"""
        self.client.login(
            email="test@user.com", password="password123")
        post_data = {
    ...
    def test_stage_two_booking_renders_correctly(self):
        ...
        self.assertIsInstance(response.context['booking_form'], ReservationForm)        
    
    def test_update_booking_renders_correctly(self):
        """ tests that logged in user can update a pending booking"""
        self.client.login(email='test@user.com', password = 'password123')
        response = self.client.get(reverse('update_booking', args=[self.pendingbooking.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_booking_posts_data_correctly(self):
        ...
        response = self.client.post(reverse('update_booking', args=[self.pendingbooking.id]), post_data)
        self.assertEqual(response.status_code, 302)
    ...
    def test_stage_two_booking_posts_data_correctly(self):
        """ Tests that logged in user can post data through the second stage of the
        booking form if everything is complete"""
        self.client.login(
            email="test@user.com", password="password123")
    ...
    def test_stage_two_booking_posts_data_states_invalid_too_many_tickets(self):
        """ Tests that logged in user cannot post data through the second stage of the
    ...
    def test_stage_two_booking_posts_data_states_invalid_one_ticket_family_session(self):
        """ Tests that logged in user cannot post data through the second stage of the
        booking form if only one ticket reserved for a family session"""
        self.client.login(
            email="test@user.com", password="password123")
        session = self.client.session
    ...
    def test_delete_booking_renders_correctly(self):
        ...
        self.client.login(email='test@user.com', password = 'password123')
        response = self.client.get(reverse('delete_booking', args=[self.pendingbooking.id]))
        self.assertEqual(response.status_code, 302)
    ...
    def test_delete_booking_posts_data_correctly(self):
        ...
        self.client.login(
            email="test@user.com", password="password123")
        response = self.client.post(reverse('delete_booking', args=[self.pendingbooking.id]))
        self.assertEqual(response.status_code, 302)
        self.assertEqual(Reservation.objects.count(),1)
    
    def test_my_bookingslist_renders_correctly(self):
        """ Tests that booking list details open reservations but not closed ones """
        self.client.login(email="test@user.com", password="password123")
    ...
    def test_my_feedbacklist_renders_correctly(self):
        """ Tests that feedback list details closed reservations but not open ones """
        self.client.login(email="test@user.com", password="password123")
    ...
    def test_leave_feedback_page_specific_session_renders_correctly(self):
        ...
        response = self.client.get(reverse('feedback_form', args=[review_id.id]))
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_feedback_posts_data_correctly(self):
        ...
        review_id = Feedback.objects.get(booking=self.oldbooking)
        response = self.client.post(reverse('feedback_form', args=[review_id.id]), post_data)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(Feedback.objects.get(pk=review_id.id).feedback_rating, 5)
        self.assertNotEqual(Feedback.objects.get(pk=review_id.id).approved, True)
