...
def mark_paid(modeladmin, request, queryset):
    """ Enables admin user to update instances of :model:`Reservation` to `paid` by bulk action. """
    queryset.update(paid = True)

@admin.action(description="Approve feedback comment for publication")
def mark_approved(modeladmin, request, queryset):
    """ Enables admin user to update instances of :model:`Feedback` to `approved` by bulk action. """
    queryset.update(approved= True)

@admin.action(description="Give user staff status")
def make_staff(modeladmin, request, queryset):
    """ Enables admin user to update instances of :model:`UserProfile` to `staff` by bulk action. """
...
class UserProfileAdmin(admin.ModelAdmin):
    """
    Renders all instances of the :model:`UserProfile` in the admin panel.
    
    Users can be marked as `staff` which will give them access to the staff area on the website when logged in
...
class FeedbackAdmin(admin.ModelAdmin):
    ...
    Feedback comments can be approved in bulk.Only approved comments will be published.
    """
